 ...
 * <a href="https://github.com/Netflix/conductor-community/blob/main/persistence/common-persistence/src/test/java/com/netflix/conductor/dao/ExecutionDAOTest.java">...</a>
 * */
...
class MongoExecutionDAOTest extends BaseMongoTest {

    @Autowired
    private MongoExecutionDAO mongoExecutionDAO;
    ...
    void setUpBeforeEach() {
        mongoTemplate.dropCollection(TaskInProgress.class);
        mongoTemplate.dropCollection(TaskDocument.class);
        mongoTemplate.dropCollection(TaskScheduled.class);
        mongoTemplate.dropCollection(WorkflowToTask.class);
        mongoTemplate.dropCollection(WorkflowDocument.class);
        mongoTemplate.dropCollection(WorkflowDefToWorkflow.class);
        mongoTemplate.dropCollection(WorkflowPending.class);
        mongoTemplate.dropCollection(EventExecutionDocument.class);
        mongoTemplate.dropCollection(PollDataDocument.class);
    }
    ...
    public void testTaskExceedsLimit() {
        ...
        taskDefinition.setName("taskdef_exceed_test_1");
        taskDefinition.setConcurrentExecLimit(1);

        WorkflowTask workflowTask = new WorkflowTask();
        workflowTask.setName("workflowTask_exceed_test_1");
        ...
        for (int i = 0; i < 15; i++) {
            ...
            task.setReferenceTaskName("taskdef_exceed_test_1");
            task.setTaskDefName("taskdef_exceed_test_1");
            tasks.add(task);
            task.setStatus(TaskModel.Status.SCHEDULED);
            task.setWorkflowTask(workflowTask);
        }

        mongoExecutionDAO.createTasks(tasks);
        assertFalse(mongoExecutionDAO.exceedsLimit(tasks.get(0)));
        tasks.get(0).setStatus(TaskModel.Status.IN_PROGRESS);
        mongoExecutionDAO.updateTask(tasks.get(0));

        for (TaskModel task : tasks) {
            assertTrue(mongoExecutionDAO.exceedsLimit(task));
        }
    }
    ...
    public void testCreateTaskException() {
        ...
        NonTransientException nonTransientException = assertThrows(NonTransientException.class, () -> {
            mongoExecutionDAO.createTasks(Collections.singletonList(task));
        }, "NonTransientException was expected");
        assertEquals("Workflow instance id cannot be null", nonTransientException.getMessage());

        task.setWorkflowInstanceId(UUID.randomUUID().toString());
        nonTransientException = assertThrows(NonTransientException.class, () -> {
            mongoExecutionDAO.createTasks(Collections.singletonList(task));
        }, "NonTransientException was expected");
        assertEquals("Task reference name cannot be null", nonTransientException.getMessage());
    }
    ...
    public void testTaskCreateDups() {
        ...
        List<TaskModel> created = mongoExecutionDAO.createTasks(tasks);
        ...
        List<TaskModel> pending = mongoExecutionDAO.getPendingTasksByWorkflow("task0", workflowId);
        assertNotNull(pending);
        assertEquals(1, pending.size());
        assertTrue(EqualsBuilder.reflectionEquals(tasks.get(0), pending.get(0)));

        List<TaskModel> found = mongoExecutionDAO.getTasks(tasks.get(0).getTaskDefName(), null, 1);
        assertNotNull(found);
    ...
    public void testTaskOps() {
        ...
        List<TaskModel> pending =
                mongoExecutionDAO.getPendingTasksForTaskType(tasks.get(0).getTaskDefName());
        assertNotNull(pending);
        ...
        for (int i = 0; i < 3; i++) {
            TaskModel found = mongoExecutionDAO.getTask(workflowId + "_t" + i);
            assertNotNull(found);
            found.getOutputData().put("updated", true);
            found.setStatus(TaskModel.Status.COMPLETED);
            mongoExecutionDAO.updateTask(found);
        }

        List<String> taskIds =
                tasks.stream().map(TaskModel::getTaskId).collect(Collectors.toList());
        List<TaskModel> found = mongoExecutionDAO.getTasks(taskIds);
        ...
        found.forEach(
                task -> {
                    ...
                    boolean removed = mongoExecutionDAO.removeTask(task.getTaskId());
                    assertTrue(removed);
                });

        found = mongoExecutionDAO.getTasks(taskIds);
    ...
    public void testPending() {
        ...
        long count = mongoExecutionDAO.getPendingWorkflowCount(def.getName());
        assertEquals(10, count);

        for (int i = 0; i < 10; i++) {
            mongoExecutionDAO.removeFromPendingWorkflow(def.getName(), workflowIds.get(i));
        }

        count = mongoExecutionDAO.getPendingWorkflowCount(def.getName());
    ...
    public void complexExecutionTest() {
        ...
        String workflowId = mongoExecutionDAO.createWorkflow(workflow);
        assertEquals(workflow.getWorkflowId(), workflowId);

        List<TaskModel> created = mongoExecutionDAO.createTasks(workflow.getTasks());
        assertEquals(workflow.getTasks().size(), created.size());

        WorkflowModel workflowWithTasks =
                mongoExecutionDAO.getWorkflow(workflow.getWorkflowId(), true);
        assertEquals(workflowId, workflowWithTasks.getWorkflowId());
        assertEquals(numTasks, workflowWithTasks.getTasks().size());

        WorkflowModel found = mongoExecutionDAO.getWorkflow(workflowId, false);
        ...
        mongoExecutionDAO.updateWorkflow(workflow);
        found = mongoExecutionDAO.getWorkflow(workflowId);
        assertNotNull(found);
    ...
    protected List<String> generateWorkflows(WorkflowModel base, int count) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            mongoExecutionDAO.createWorkflow(base);
